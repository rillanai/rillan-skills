...
This skill is the release-integrity layer. Pair it with `core.md` for pipeline principles, the platform-specific CI/CD skills (`github-actions.md`, `azure-devops.md`) for wiring, and `security` for threat analysis.
...
- For CI actions, pin **by commit SHA** (see `github-actions.md`).
