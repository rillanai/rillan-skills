...
This skill is the GitHub Actions specialization layer. Pair it with `core.md` for platform-agnostic pipeline principles and with `supply-chain.md` for release integrity.
...
- For SBOM, signature, and provenance, use `actions/attest-build-provenance` (GitHub's native attestation) or cosign — see `supply-chain.md`.
