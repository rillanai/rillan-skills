...
This skill is the platform-agnostic layer. It defines what good pipelines look like: reproducibility, feedback speed, secret discipline, artifact integrity, and least-privileged credentials. Pair it with a platform-specific skill (`github-actions.md`, `azure-devops.md`) and, for deployment, with `gitops.md`.
...
- Do not mix deploy steps into CI; deployment belongs in CD (pull-based GitOps is preferred — see `gitops.md`).
...
- Attach SBOM, signature, and provenance to every published artifact. See `supply-chain.md`.
