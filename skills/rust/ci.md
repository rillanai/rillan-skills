...
This skill is the Rust-specific CI layer. Pair it with `cicd/core.md` for platform-agnostic principles, a platform-specific skill (`cicd/github-actions.md` or `cicd/azure-devops.md`) for wiring, `cicd/supply-chain.md` for release integrity, and `test.md` / `policy.md` for test-shape and quality rules.
...
msrv:
  ...
  steps:
    ...
    - uses: actions-rust-lang/setup-rust-toolchain@<sha>
      with:
        toolchain: "1.X"   # match `version.md` in Cargo.toml
    - uses: Swatinem/rust-cache@<sha>
    - name: Check on MSRV
