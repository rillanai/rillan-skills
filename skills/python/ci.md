...
This skill is the Python-specific CI layer. Pair it with `cicd/core.md` for platform-agnostic principles, a platform-specific skill (`cicd/github-actions.md` or `cicd/azure-devops.md`) for wiring, `cicd/supply-chain.md` for release integrity, and `test.md` / `policy.md` for test-shape and quality rules.
...
### 9. CI Summary
Publish a summary of all job results on main-branch pushes. Same pattern as Go CI — see `go/ci.md`.
...
This replaces long-lived PyPI API tokens entirely — the gold standard for Python publishing. See `cicd/supply-chain.md` for signing and provenance.
...
- Pin the action/task SHAs (see `cicd/github-actions.md` / `cicd/azure-devops.md`).
