---
name: rfc
description: Use to author, review, or revise Request for Comments (RFC) documents — proposals for non-trivial cross-team or hard-to-reverse changes that need discussion before commitment. RFC is upstream of an ADR (proposal vs. decision). Skip for already-decided choices (use `adr`) or single-team implementation that can be settled in a PR.
---
...
Use this skill for RFCs. Use `adr` for ADRs.
...
## Relationship To ADR
- When an RFC is **accepted**, write an ADR (use `adr`) that captures the decision concisely. The ADR points to the RFC for context; the RFC points to the ADR for resolution.
