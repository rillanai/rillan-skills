---
name: docker
description: Use when writing or reviewing Dockerfiles or OCI container images — base selection (scratch → distroless → slim), multi-stage builds, non-root user, layer discipline, BuildKit secret handling, SBOM, signing, multi-arch, registry labels. Pin bases by digest. Pair with `kubernetes/dev.md` for runtime concerns.
---
...
This skill focuses on the image itself. For how the image is *run* inside a cluster, pair it with `kubernetes/dev.md`.
